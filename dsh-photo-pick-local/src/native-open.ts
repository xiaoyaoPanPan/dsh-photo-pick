 ...
 */
function powershellEncodedArgs(script: string): readonly string[] {
 ...
 */
export async function revealPhotoPickPath(
  path: string,
  signal: AbortSignal,
 ...
 */
export function isWindowsExplorerBogusFailure(error: unknown): boolean {
 ...
 * @param run - injectable runner (tests).
 */
export async function openPhotoPickPath(
  path: string,
  signal: AbortSignal,
  run: NativeCommandRunner = runNativeCommand,
): Promise<void> {
