...
import { parseVisionScoreJson, truncateVisionResponse } from './parse-score.ts'
import { DEFAULT_VISION_MAX_EDGE_PX, prepareVisionImage } from './vision-image.ts'
import { isRateLimitReason } from './vision-throttle.ts'
 ...
 */
export function resolveScorePrompt(custom?: string, criteria?: string): string {
 ...
 */
export async function scoreImageWithLlm(
  filePath: string,
  relativePath: string,
  config: ScoreImageConfig,
  signal?: AbortSignal,
): Promise<ScoreImageResult> {
  const base: PhotoPickScore = {
    path: filePath,
    relativePath,
    score: 0,
    reasons: [],
  ...
  try {
    ...
    const prepared = await prepareVisionImage(
      bytes,
      mediaType,
      config.maxEdgePx ?? DEFAULT_VISION_MAX_EDGE_PX,
    )
...
function mediaTypeOf(filePath: string): ImageMediaType | undefined {
  return MEDIA_TYPES[extname(filePath).toLowerCase()]
}

async function collectAssistantText(stream: AsyncIterable<StreamChunk>): Promise<string> {
  let text = ''
