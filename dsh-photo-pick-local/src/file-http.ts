 ...
 */
export function registerPhotoPickFileHttp(ctx: Context): () => void {
...
function parseQuery(url: string | undefined): { root: string; relativePath: string } | undefined {
  ...
  if (relativePath === null || relativePath.trim().length === 0) return undefined
