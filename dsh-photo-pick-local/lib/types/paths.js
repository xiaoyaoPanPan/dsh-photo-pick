 ...
 */
export function assertAllowedRoot(canonical) {
    ...
    if (normalized.toLowerCase() === home.toLowerCase()) {
        throw new PhotoPickError('refusing to use the user home directory as a photo-pick root', 'INVALID_ROOT');
    }
    const driveRoot = trimSep(parse(canonical).root);
    if (driveRoot !== '' && normalized.toLowerCase() === driveRoot.toLowerCase()) {
        throw new PhotoPickError('refusing to use a drive or filesystem root as a photo-pick root', 'INVALID_ROOT');
    }
}
/**
 ...
 */
export async function resolveContainedPath(root, requested) {
