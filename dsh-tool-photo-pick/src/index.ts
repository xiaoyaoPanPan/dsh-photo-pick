...
import type { PhotoPickError, PhotoPickResult, PhotoPickScore } from 'dsh-photo-pick'
import type {} from 'dsh-photo-pick'
...
function formatScore(row: PhotoPickScore, index: number): string {
...
function formatResult(result: PhotoPickResult): string {
...
function toRankMeta(row: PhotoPickScore): PhotoPickRankRowMeta {
  return {
...
function toToolValue(result: PhotoPickResult): PhotoPickToolValue {
  return {
...
function formatError(error: unknown): string {
  if (error && typeof error === 'object' && 'code' in error && 'message' in error) {
    const pick = error as PhotoPickError
    return `${pick.code}: ${pick.message}`
  }
  return error instanceof Error ? error.message : String(error)
}

function pathsCall(title: string, paths: readonly string[]): GenericCallView {
  return {
    card: 'generic',
    title,
    kind: 'other',
...
function presentPhotoPickResult(
  _args: unknown,
  result: ToolResult,
...
export function apply(ctx: Context): void {
  ...
  ctx.tools.register(defineTool({
    ...
    description:
      'Rank similar photos under the current workspace and recommend the best ones. '
    ...
    parameters: {
      ...
      criteria: {
        type: 'string',
        description:
          'Per-batch preference from the user (Criteria / 择优要求). '
