...
function requireCwd(exec) {
	...
	if (cwd === void 0 || cwd === "") throw new Error("photo-pick tools require an agent session with a workspace cwd");
	return cwd;
}
...
function formatResult(result) {
	const picks = result.picks.length === 0 ? "(no successful scores)" : result.picks.map((row, index) => formatScore(row, index)).join("\n\n");
	return [
...
function apply(ctx) {
	...
	ctx.tools.register(defineTool({
		...
		description: "Rank similar photos under the current workspace and recommend the best ones. Pass paths (absolute or workspace-relative image files) for an explicit burst, and/or query to search the media library when that plugin is installed. Uses a separate vision model (photo-pick settings: visionLlmProvider + visionModel). Returns top-K picks with scores, reasons, and flaws — do not call read_image.",
		parameters: {
			paths: {
				type: "array",
			...
			criteria: {
				type: "string",
				description: "Per-batch preference from the user (Criteria / 择优要求). Pass the user text unchanged, e.g. no bare legs / head in upper third."
			}
		},
		output: {
			schema: {
		...
		presentCall: (args) => {
			...
			return pathsCall(paths.length > 0 ? `Pick best of ${paths.length} photos` : typeof args.query === "string" && args.query.length > 0 ? `Pick best: ${args.query}` : "Pick best photos", paths);
		},
		presentResult: presentPhotoPickResult,
		async execute(args, exec) {
			try {
				const paths = Array.isArray(args.paths) ? args.paths.filter((p) => typeof p === "string" && p.length > 0) : void 0;
				const query = typeof args.query === "string" ? args.query : void 0;
				if ((paths === void 0 || paths.length === 0) && (query === void 0 || query.trim() === "")) throw new Error("photo_pick_best requires paths[] and/or query");
				return toToolValue(await ctx.photoPick.pickBest(requireCwd(exec), {
					...paths !== void 0 && paths.length > 0 ? { paths } : {},
					...query !== void 0 && query.trim() !== "" ? { query } : {},
					...typeof args.topK === "number" ? { topK: args.topK } : {},
					...typeof args.maxCandidates === "number" ? { maxCandidates: args.maxCandidates } : {},
					...typeof args.criteria === "string" && args.criteria.trim() !== "" ? { criteria: args.criteria } : {},
					signal: exec.signal
				}));
			} catch (error) {
...
//#endregion
export { PHOTO_PICK_PROMPT, apply, inject, name };
